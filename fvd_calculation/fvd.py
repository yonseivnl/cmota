...
from loader import VideoGenerateDataset, VideoGenerateInstanceDataset
import random
import os
...
def calculate_fvd_from_inference_result(gen_path, ref_path='cha_sampled_post', num_of_video=16, video_length=10):

  VIDEO_LENGTH = video_length
  print('{}'.format(video_length))
  base_ref = VideoGenerateDataset(ref_path, min_len=VIDEO_LENGTH)
  base_tar = VideoGenerateInstanceDataset(gen_path, min_len=VIDEO_LENGTH)
  ...
  print("length::::::", len(videoloader_ref), len(videoloader_tar))
  ...
  print("images ref tar shape: ", images_ref.shape, images_tar.shape) # (16, 10, 64, 64, 3) (16, 10, 64, 64, 3)

  with tf.Graph().as_default():
    ref_tf = tf.convert_to_tensor(images_ref, dtype=tf.uint8)
    tar_tf = tf.convert_to_tensor(images_tar, dtype=tf.uint8)

    first_set_of_videos = ref_tf
    second_set_of_videos = tar_tf

    result = calculate_fvd(create_id3_embedding(preprocess(first_set_of_videos,  (224, 224)), bs),
                           create_id3_embedding(preprocess(second_set_of_videos, (224, 224)), bs))
...
if __name__ == "__main__":
    print(calculate_fvd_from_inference_result(gen_path='qualitative/test_good_2'))
