...
parser.add_argument("-I", "--image_only", action='store_true', help="Check only images.", required=False)
...
def return_ids(files):
    ids = [x[:-4] for x in files]
    d = collections.defaultdict(int)
    for x in ids: d[x] += 1
    paired_ids = list(set(ids))

    return {'paired_ids': paired_ids}
...
def return_corrupt_images(parent, ids):
    corrupt_images = []
    delete = []
    ...
    return {'corrupt': corrupt_images, 'delete': delete}
...
for folder in folders:
    ...
    if args.image_only:
        ...
        incomplete_and_paired_files_in_parent_folder = return_ids(files)  
        empty_and_corrupt_files_in_parent_folder = \
            return_corrupt_images(
                parent, 
